...
def arm_scope(alias: str, channels: str = "1") -> bool:
    """Arm one acquisition and return immediately (non-blocking), for an event in a LATER step.

    Use with a NORMal-trigger config when the signal is generated by a step AFTER this one:
    arm_scope here, command the event in the next step, then read_scope to pull the frozen
    record. Returns True once armed. Tektronix.
    """
    return _fleet.arm(alias, channels)


def read_scope(alias: str, points: int = 0, timeout_s: float = 120.0) -> bool:
    """Read the record from a prior arm_scope() once the trigger has fired. Tektronix.

    Waits up to timeout_s for the armed acquisition to complete, then reads every armed channel
    off the frozen record (no re-arm, no AUTO - the captured event is preserved). Then use the
    scope_*/save_* calls. Raises TimeoutError if the trigger never fires within timeout_s.
    """
    return _fleet.read_armed(alias, int(points), float(timeout_s))
